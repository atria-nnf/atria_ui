...
import { deleteService, toggleServiceFeatured } from '@/lib/api/admin/services'
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-xl p-6 max-w-md w-full mx-4 shadow-xl">
            ...
            <p className="mt-2 text-gray-600">
              Jeste li sigurni da želite obrisati uslugu &quot;{name}&quot;? Ova
...
function FeaturedToggle({ id, featured }: { id: string; featured: boolean }) {
  ...
  const handleToggle = async () => {
    ...
    const { error } = await toggleServiceFeatured(id, newValue)
    setLoading(false)

    if (error) {
      setIsFeatured(!newValue) // Revert on error
      alert(`Greška: ${error}`)
    } else {
      router.refresh()
    }
